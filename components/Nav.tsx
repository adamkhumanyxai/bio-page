...
const NAV_LINKS = [
  ...
  ['Off the Clock', '#off-the-clock'], ['Contact', '#contact'],
] as const;

const SECTION_IDS = ['story', 'timeline', 'projects', 'off-the-clock', 'contact'];
...
export default function Nav({ onTalk }: Props) {
  ...
  return (
    <div style={{ position: 'sticky', top: 0, zIndex: 50, background: 'rgba(255,255,255,0.92)', backdropFilter: 'blur(10px)', borderBottom: '1px solid var(--rule)' }}>
      <div className="navInner" style={{ maxWidth: 1520, margin: '0 auto', padding: '20px 40px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        ...
        <div className="navLinks" style={{ display: 'flex', gap: 32, fontSize: 14 }}>
        ...
        <button className="navTwinCta" onClick={onTalk} style={{ padding: '11px 18px 11px 14px', background: 'var(--bg-ink)', color: '#fff', border: 'none', borderRadius: 999, fontSize: 13, fontWeight: 600, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 10 }}>
